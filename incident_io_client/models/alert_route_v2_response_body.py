...
if TYPE_CHECKING:
    from ..models.alert_route_escalation_binding_v2_response_body import (
        AlertRouteEscalationBindingV2ResponseBody,
    )
    from ..models.alert_route_incident_template_v2_response_body import (
    ...
    from ..models.grouping_key_v2_response_body import GroupingKeyV2ResponseBody


T = TypeVar("T", bound="AlertRouteV2ResponseBody")


@_attrs_define
class AlertRouteV2ResponseBody:
    """
    Example:
    ...
    Attributes:
        condition_groups (List['ConditionGroupV2ResponseBody']): What condition groups must be true for this alert route
            to fire? Example: [{'conditions': [{'operation': {'label': 'Lawrence Jones', 'value':
            '01FCQSP07Z74QMMYPDDGQB9FTG'}, 'param_bindings': [{'array_value': [{'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal': 'SEV123',
            'reference': 'incident.severity'}}], 'subject': {'label': 'Incident Severity', 'reference':
            'incident.severity'}}]}].
        defer_time_seconds (int): How long should the escalation defer time be? Example: 1.
        escalation_bindings (List['AlertRouteEscalationBindingV2ResponseBody']): Which escalation paths should this
            alert route escalate to? Example: [{'binding': {'array_value': [{'label': 'Lawrence Jones', 'literal': 'SEV123',
            'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal': 'SEV123', 'reference':
            'incident.severity'}}}].
        grouping_keys (List['GroupingKeyV2ResponseBody']): Which attributes should this alert route use to group alerts?
        ...
        expressions (Union[Unset, List['ExpressionV2ResponseBody']]): The expressions used in this template Example:
        ...
        template (Union[Unset, AlertRouteIncidentTemplateV2ResponseBody]):  Example: {'custom_field_priorities':
            {'abc123': 'first-wins'}, 'custom_fields': {'custom_field_10014': {'array_value': [{'label': 'Lawrence Jones',
            'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}}}, 'incident_mode': {'array_value': [{'label': 'Lawrence Jones',
            'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}}, 'incident_type': {'array_value': [{'label': 'Lawrence Jones',
            'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}}, 'name': {'array_value': [{'label': 'Lawrence Jones', 'literal':
            'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones', 'literal': 'SEV123',
            'reference': 'incident.severity'}}, 'priority_severity': 'severity-first-wins', 'severity': {'array_value':
            [{'label': 'Lawrence Jones', 'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'label':
            'Lawrence Jones', 'literal': 'SEV123', 'reference': 'incident.severity'}}, 'summary': {'array_value': [{'label':
            'Lawrence Jones', 'literal': 'SEV123', 'reference': 'incident.severity'}], 'value': {'label': 'Lawrence Jones',
            'literal': 'SEV123', 'reference': 'incident.severity'}}, 'workspace': {'array_value': [{'label': 'Lawrence
    ...
    escalation_bindings: List["AlertRouteEscalationBindingV2ResponseBody"]
    grouping_keys: List["GroupingKeyV2ResponseBody"]
    grouping_window_seconds: int
    id: str
    name: str
    expressions: Union[Unset, List["ExpressionV2ResponseBody"]] = UNSET
    template: Union[Unset, "AlertRouteIncidentTemplateV2ResponseBody"] = UNSET
    ...
    def from_dict(cls: Type[T], src_dict: Dict[str, Any]) -> T:
        ...
        for escalation_bindings_item_data in _escalation_bindings:
            escalation_bindings_item = AlertRouteEscalationBindingV2ResponseBody.from_dict(
        ...
        for grouping_keys_item_data in _grouping_keys:
            grouping_keys_item = GroupingKeyV2ResponseBody.from_dict(grouping_keys_item_data)
        ...
        template: Union[Unset, AlertRouteIncidentTemplateV2ResponseBody]
        if isinstance(_template, Unset):
            template = UNSET
        else:
            template = AlertRouteIncidentTemplateV2ResponseBody.from_dict(_template)

        alert_route_v2_response_body = cls(
            condition_groups=condition_groups,
            defer_time_seconds=defer_time_seconds,
            escalation_bindings=escalation_bindings,
            grouping_keys=grouping_keys,
        ...
        alert_route_v2_response_body.additional_properties = d
        return alert_route_v2_response_body
