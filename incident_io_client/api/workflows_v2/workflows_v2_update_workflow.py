...
from ...models.workflows_v2_update_workflow_request_body import (
...
def _get_kwargs(
    id: str,
    *,
    body: WorkflowsV2UpdateWorkflowRequestBody,
) -> Dict[str, Any]:
...
def sync_detailed(
    ...
    *,
    ...
    Args:
        id (str):
        body (WorkflowsV2UpdateWorkflowRequestBody):  Example: {'workflow': {'annotations':
            {'abc123': 'abc123'}, 'condition_groups': [{'conditions': [{'operation': 'one_of',
            'param_bindings': [{'array_value': [{'literal': 'SEV123', 'reference':
            'incident.severity'}], 'value': {'literal': 'SEV123', 'reference': 'incident.severity'}}],
            'subject': 'incident.severity'}]}], 'continue_on_step_error': True, 'delay':
            ...
            'incident.status'}], 'folder': 'My folder 01', 'include_private_incidents': True,
            'managed_source_url': 'https://github.com/my-company/incident-io-workflows', 'name': 'My
            workflow', 'once_for': ['incident.url'], 'runs_on_incident_modes': ['standard',
